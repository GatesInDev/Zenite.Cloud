...
function EmailForm({ source = 'hero', size = 'large', c, sh }) {
  ...
  if (status === 'success' || status === 'already') {
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(39,194,129,0.1)', border: `1px solid rgba(39,194,129,0.3)`, borderRadius: 4, padding: '14px 20px', maxWidth: big ? 480 : 400 }}>
  ...
  return (
    <form onSubmit={submit} style={{ width: '100%', maxWidth: big ? 480 : 400 }}>
      <div style={{ display: 'flex', gap: 6, background: c.card, border: `1px solid ${c.border}`, borderRadius: 4, padding: 5, boxShadow: sh.s1 }}>
        <input
          type="email" required value={email} onChange={e => setEmail(e.target.value)}
          placeholder="seu@email.com.br"
          style={{ flex: 1, background: 'none', border: 'none', outline: 'none', color: c.text, fontSize: big ? 15 : 13, padding: big ? '8px 12px' : '6px 10px', fontFamily: 'inherit' }}
        />
    ...
    <>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, background: c.surface, borderBottom: `1px solid ${c.border}`, padding: '0 5%', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: sh.s1, transition: 'background-color 0.32s ease, border-color 0.32s ease' }}>
        ...
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          ...
          <button onClick={toggleTheme} title={isDark ? 'Mudar para claro' : 'Mudar para escuro'} style={{ width: 34, height: 34, borderRadius: 4, background: 'transparent', border: `1px solid ${c.border}`, color: c.textSub, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'border-color 0.15s, color 0.15s' }}
      ...
      <section style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '120px 5% 80px', position: 'relative', overflow: 'hidden', background: c.base, transition: 'background-color 0.32s ease' }}>
        ...
        <div className="hero-1" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: c.card, border: `1px solid ${c.borderHard}`, borderRadius: 4, padding: '6px 16px', marginBottom: 28, boxShadow: sh.s1 }}>
          <span style={{ width: 6, height: 6, borderRadius: '50%', background: c.green }} />
          <span style={{ fontSize: 13, fontWeight: 600, color: c.blue4 }}>Acesso Antecipado — Startup Weekend Chapecó 2026</span>
        </div>

        <h1 className="hero-2" style={{ fontSize: 'clamp(36px,5vw,68px)', fontWeight: 900, letterSpacing: -2, textAlign: 'center', lineHeight: 1.05, maxWidth: 800, marginBottom: 20 }}>
        ...
        <p className="hero-3" style={{ fontSize: 18, color: c.textSub, textAlign: 'center', maxWidth: 560, marginBottom: 40, lineHeight: 1.6 }}>
        ...
        <div className="hero-5" style={{ display: 'flex', gap: 48, marginTop: 56, flexWrap: 'wrap', justifyContent: 'center' }}>
          {[
      ...
      <section style={{ padding: '80px 5%', background: c.surface, borderTop: `1px solid ${c.border}`, borderBottom: `1px solid ${c.border}`, transition: 'background-color 0.32s ease' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          ...
          <h2 data-reveal data-d="1" style={{ fontSize: 'clamp(28px,3.5vw,44px)', fontWeight: 800, letterSpacing: -1, marginBottom: 16, lineHeight: 1.15, color: c.text }}>
            Seus arquivos estão no WhatsApp,<br />
      ...
      <section id="features" style={{ padding: '80px 5%', background: c.base, transition: 'background-color 0.32s ease' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          ...
          <h2 data-reveal data-d="1" style={{ fontSize: 'clamp(28px,3.5vw,44px)', fontWeight: 800, letterSpacing: -1, marginBottom: 16, lineHeight: 1.15, color: c.text }}>
            Um cofre digital que protege,<br />
      ...
      <section id="preview" style={{ padding: '80px 5%', background: c.surface, borderTop: `1px solid ${c.border}`, borderBottom: `1px solid ${c.border}`, transition: 'background-color 0.32s ease' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          ...
          <h2 data-reveal data-d="1" style={{ fontSize: 'clamp(28px,3.5vw,44px)', fontWeight: 800, letterSpacing: -1, marginBottom: 40, lineHeight: 1.15, color: c.text }}>
            Veja como o Zênite funciona
          </h2>

          <div data-reveal data-d="2" style={{ display: 'flex', gap: 6, marginBottom: 20, flexWrap: 'wrap' }}>
            {[
      ...
      <section style={{ padding: '80px 5%', background: c.base, transition: 'background-color 0.32s ease' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          ...
          <div data-reveal data-d="2" style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 600 }}>
              <thead>
                <tr style={{ background: c.card }}>
                  ...
                  <th style={{ textAlign: 'center', padding: '12px 16px', fontSize: 12, color: c.textSub,  borderBottom: `2px solid ${c.border}`, fontWeight: 500 }}>Google Drive</th>
                  <th style={{ textAlign: 'center', padding: '12px 16px', fontSize: 12, color: c.textSub,  borderBottom: `2px solid ${c.border}`, fontWeight: 500 }}>Dropbox</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  { feat: 'Classificação por IA (CPF, RG…)', z: true, g: false,     d: false     },
                  ...
                  <tr key={row.feat} style={{ borderBottom: `1px solid ${c.border}` }}>
                    <td style={{ padding: '12px 16px', fontSize: 14, color: c.textSub }}>{row.feat}</td>
      ...
      <section id="cta" style={{ padding: '80px 5% 100px', background: c.surface, borderTop: `1px solid ${c.border}`, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', transition: 'background-color 0.32s ease' }}>
        ...
        <h2 data-reveal data-d="1" style={{ fontSize: 'clamp(28px,4vw,52px)', fontWeight: 900, letterSpacing: -1.5, marginBottom: 16, lineHeight: 1.1, color: c.text }}>
      ...
      {/* ── FOOTER ───────────────────────────────────────── */}
      <footer style={{ padding: '28px 5%', borderTop: `1px solid ${c.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, background: c.base, transition: 'background-color 0.32s ease' }}>
        <Logo size={22} c={c} />
